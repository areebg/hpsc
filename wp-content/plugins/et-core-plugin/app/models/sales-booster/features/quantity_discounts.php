...
if ( 'yes' !== get_option( 'woocommerce_enable_ajax_add_to_cart', 'yes' ) ) {
    ...
    <p class="et-message et-warning">
        <?php
            echo sprintf(esc_html__('To properly use this feature, please activate %s option in your WooCommerce settings.', 'xstore'), '<a href="'.admin_url('admin.php?page=wc-settings&tab=products').'" target="_blank" rel="nofollow">'. esc_html__('Enable AJAX add to cart buttons on archives', 'xstore') . '</a>')
        ?></p>
    <?php
}
